...
def wrangle_inmate_data_from_tag(rowtag):
    ...
    dict = {"tdcj_id": td_tags[0].text.strip(),
        "url": make_absolute_url(td_tags[1].select('a')[0].attrs['href']),
        "last_name": td_tags[2].text.strip(),
        "first_name": td_tags[3].text.strip(),
        "birthdate": txdate_to_iso(td_tags[4].text.strip()),
        "gender": td_tags[5].text.strip(),
        "race": td_tags[6].text.strip(),
        "date_received": txdate_to_iso(td_tags[7].text.strip()),
        "county": td_tags[8].text.strip(),
        "date_offense": txdate_to_iso(td_tags[9].text.strip()),
        "age_at_offense": calc_years_diff(txdate_to_iso(td_tags[4].text.strip()), txdate_to_iso(td_tags[9].text.strip())),
        "years_before_death_row": calc_years_diff(txdate_to_iso(td_tags[9].text.strip()), txdate_to_iso(td_tags[7].text.strip()))}
    return dict
